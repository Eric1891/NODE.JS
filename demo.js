...
app.get ("/MacBookPro", function (req,res)
{
 ...
 res.render ( "MacBookPro.ejs",{message:"",flag:"1"});
 else
 {
...
{
 if(req.session.flag =="1")
 res.render ( "HP.ejs",{message:"",flag:"1"});
 else
 {
 req.flash('message', 'ERROR: Must Login First');
 res.redirect ( "login");
...
{
 if(req.session.flag =="1")
 res.render ( "Dell.ejs",{message:"",flag:"1"});
 else
 {
 req.flash('message', 'ERROR: Must Login First');
 res.redirect ( "login");
...
{
    if(req.session.flag =="1") res.render ( "about.ejs",{message:"",flag:"1"});
    else
    {   req.flash('message', 'ERROR: Must Login First');
        res.redirect ( "login");
    }	
...
{
	if(req.session.flag =="1") res.render ( "products.ejs",{message:"",flag:"1"});	
    else 
    {   req.flash('message', 'ERROR: Must Login First');
        res.redirect ( "login");
    }	
...
{
    if(req.session.flag =="1") res.render ( "contact.ejs",{message:"",flag:"1"});	
    else
    {   req.flash('message', 'ERROR: Must Login First');
        res.redirect ( "login");
    }	
    ...
    User.findOne({userid:req.query.userid}, '', function (err, data)
    {
        ...
        {       
            ...
            //{
                ...
                { 
                    if (err) return handleError(err);
                    //const msg="User Stored";
                    //res.render("login.ejs",{message:msg,flag:""});
        ...
        {
            //let msg="User Already In Database"
            //res.render("registration.ejs",{message:msg,flag:""});
...
app.get("/loginx", (req, res) =>
{
    User.findOne({userid:req.query.userid}, '', function (err, data)
    {
        ...
        if (data==null)
        {
            ...
            //res.render ( "login.ejs",{message:message,flag:""});
...
app.post("/add", (req, res) => 
{
    ...
    Cart.find(item2find, '', function (err, data) 
    {
		...
		else
		{
			...
			//Cart.updateOne({code: "lgtv"}, { quantity: "20" },function(err,result);
...
app.get("/cart", (req, res) => 
{
    ...
    Cart.find(item2find, '', function (err, data) 
    {
		...
		else
		{
			...
			{
				var image="images/"+data[i].code+".jpeg";
			    cart+="<tr><td><img style='width:60px' src='"+image+"' /></td><td>"+data[i].name+"</td><td><div style='vertical-align: middle;'><form style='float:left' method=post action='/change'><input style='float:left' type=hidden name=code value='"
								...
								+"<a href='/delete:"+data[i].code+"' style='float:left'><img src='images/x.png' style='width:20px' style='float:left'/></a></div></td><td class='right'>$"
